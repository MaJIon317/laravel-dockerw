        ...
        <div class="container">
            ...
            <div class="section__title">
                <div class="section__title-item">
                    @foreach($tabs as $tab)
                        <h2 class="section__title-h @if($tab_active == $tab) active @endif" @if($tab_active != $tab) wire:click="select('{{$tab}}')" @endif>{{ __('catalog.' . $tab) }}</h2>
                    @endforeach
                </div>
                <div class="section__title-item">
                    @if($all_url)
                        <a href="{{ $all_url }}" class="btn section__title-link" wire:navigate>Посмотреть все</a>
                    @endif
                    <div class="section__title-buttons">
                        <div class="swiper-button-prev swiper-button--pos" data-slider-prev="tab{{ $tab_active }}">
                        ...
                        <div class="swiper-button-next swiper-button--pos" data-slider-next="tab{{ $tab_active }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="17" height="14" viewBox="0 0 17 14">
            ...
            @if($products && count($products) > 0)
                <div class="swiper-section sectionproduct">
                    <div class="swiper" data-slider-design="sectionproduct" data-slider-id="tab{{ $tab_active }}">
                        <div class="swiper-wrapper">
                            @foreach($products as $product)
                                <div class="swiper-slide">
                                    <div class="productcard" wire:key="{{ 'productcard-tab-' . $tab_active . $product->id }}">
                                        <livewire:productCard :product="$product" wire:key="{{ 'productcard-tab-' . $tab_active . $product->id }}" />
                                    </div>                  
                                </div>
                            @endforeach
                        </div>
