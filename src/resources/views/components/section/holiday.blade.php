                                        ...
                                        <div class="sectionholiday__item">
                                            <div class="sectionholiday__item-block">
                                                <div class="sectionholiday__item-image">
                                                    <a href="{{ route('category', $holiday->slug) }}" class="sectionholiday__item-a" wire:navigate>
                                                        <img src="{{ resize($holiday->image, 260, 260) }}" alt="{!! $holiday->title !!}" class="sectionholiday__item-img">
                                                    </a>
                                                    <span class="sectionholiday__item-time sectionholiday__item-time--min">
                                                        @if($current_month == $holiday->month && $current_day == $holiday->day)
                                                            сегодня
                                                        @else
                                                            {{ \Carbon\Carbon::createFromFormat('Y.m.d', ((($current_month == $holiday->month && $current_day <= $holiday->day) || ($current_month < $holiday->month)) ? $current_year : $next_year) . '.' . $holiday->month . '.' . $holiday->day)->diffForHumans() }}
                                                        @endif
                                                    </span>
                                                </div>
                                                <h5 class="sectionwork__item-name">
                                                    <a href="{{ route('category', $holiday->slug) }}" wire:navigate>{!! $holiday->title !!}</a>
                                                </h5>
                                            </div>
                                        </div>
                                    </div>
